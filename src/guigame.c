...
static void guiGameSetCheatSettingsState(void)
{
    ...
    diaSetEnabled(diaCheatConfig, CHTCFG_SELECT, EnableCheat);
}
...
static void guiGameShowSelectCheatsDialog(int id, item_list_t *support)
{
    if (!support || !support->itemGetStartup)
        return;

    char *startup = support->itemGetStartup(support, id);
    if (!startup || strlen(startup) == 0)
        return;

    char prefix[64] = "";
    if (support->itemGetPrefix) {
        char *p = support->itemGetPrefix(support);
        if (p)
            strncpy(prefix, p, sizeof(prefix) - 1);
    }

    char cheatfile[256];
    snprintf(cheatfile, sizeof(cheatfile), "%sCHT/%s.cht", prefix, startup);

    cheat_file_t cheatFile;
    int count = ParseCheatFileItems(cheatfile, &cheatFile);
    if (count <= 0) {
        guiMsgBox("File cheat tidak ditemukan di CHT/<GameID>.cht!", 0, NULL);
        return;
    }

    // Allocate dynamic UI items: Header (2) + per-cheat (4) + Button (3) + Terminator (1)
    int maxUiItems = count * 4 + 10;
    struct UIItem *diaItems = malloc(sizeof(struct UIItem) * maxUiItems);
    if (!diaItems)
        return;

    memset(diaItems, 0, sizeof(struct UIItem) * maxUiItems);

    int idx = 0;
    diaItems[idx++] = (struct UIItem){UI_LABEL, 0, 1, 1, -1, 0, 0, {.label = {"PILIH KODE CHEAT AKTIF (ON / OFF)", -1}}};
    ...
    diaItems[idx++] = (struct UIItem){UI_BREAK};
    diaItems[idx++] = (struct UIItem){UI_OK, 0, 1, 1, -1, 0, 0, {.label = {NULL, _STR_OK}}};
    diaItems[idx++] = (struct UIItem){UI_BREAK};
    diaItems[idx++] = (struct UIItem){UI_TERMINATOR};

    int result = diaExecuteDialog(diaItems, -1, 1, NULL);
    if (result == UI_OK) {
        for (int i = 0; i < count; ++i) {
            if (!cheatFile.items[i].is_mastercode) {
                diaGetInt(diaItems, CHT_ITEM_BASE + i, &cheatFile.items[i].enabled);
            } else {
                cheatFile.items[i].enabled = 1;
            }
        }
        SaveCheatFileItems(cheatfile, &cheatFile);
        guiMsgBox("Pengaturan cheat berhasil disimpan!", 0, NULL);
    }

    free(diaItems);
}

void guiGameShowCheatConfig(int id, item_list_t *support)
{
    // configure the enumerations
    const char *settingsSource[] = {_l(_STR_GLOBAL_SETTINGS), _l(_STR_PERGAME_SETTINGS), NULL};
    ...
    int result = -1;
    do {
        result = diaExecuteDialog(diaCheatConfig, result, 1, &guiGameCheatUpdater);
        if (result == CHTCFG_SELECT || result == CHTCFG_SELECT_CHEATS) {
            guiGameShowSelectCheatsDialog(id, support);
        }
    } while (result >= COMPAT_NOEXIT);
}
